  ...
  function mixin(namespace, globalOptions) {
    ...
    _.extend(fn, {
      ...
      parseAssociation: function(associationType, associationName, options) {
        ...
        if (options.parse) {
          ...
          if (associations[associationType]) {
            ...
            if (_.isEmpty(parsers)) {
              this.prototype.parse = _.wrap(this.prototype.parse, function wrappedParse(oldParse, response) {
                // console.time('wrappedParse')
                var self = this;

                var parsedResponse = oldParse.apply(self, _.rest(arguments));

                for (var i = 0; i < parsers.length; i++) {
                  var parser = parsers[i];
                  var result = parser.parseFn.call(self, parsedResponse);
                  parser.associationFn.call(self, result.response)

                  if (result.key) {
                    delete parsedResponse[result.key];
                  }
                }
                return parsedResponse;


                // var retval = _.tap(oldParse.apply(self, _.rest(arguments)), function(parsedResponse) {
                //   _.chain(parsers)
                //       .map(function(parser) {
            ...
            if (_.isFunction(options.parse)) {
              parserFn = function(response) { return {response: options.parse.call(this, response) } };
            } else {
              parserFn = function(response) {
        ...
        // console.timeEnd('parseAssociation');
      }
    });
